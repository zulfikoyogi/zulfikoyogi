...
if(isset($_POST['login'])) {
    @$sql = mysqli_query($koneksi, "SELECT `guru`.*, `mapel`.`nama` as nama_mapel  FROM `guru` INNER JOIN `mapel` ON `guru`.`id_mapel` = `mapel`.`id` WHERE username ='$_POST[user]' and password = '$_POST[pass]'");
    ...
    if($cek > 0) {
        ...
        $_SESSION['id_mapel'] = $row['id_mapel'];
        $_SESSION['nama_mapel'] = $row['nama_mapel'];
